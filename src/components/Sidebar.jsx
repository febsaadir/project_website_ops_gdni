...
import { 
  ...
  ShieldCheck, Smartphone, PackageX 
} from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import gdnLogo from '../assets/logo.png'; // Pastikan nama file logo benar
  ...
  const menuGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'D.O.A Return', path: '/doa', icon: PackageX } // SEDERHANAKAN PATH JADI /doa
      ]
    }
  ];
    ...
    >
      ...
      <div className="h-24 flex items-center justify-between px-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
              <h1 className="text-lg font-bold tracking-tight text-slate-800 dark:text-white leading-tight">GDNi Ops</h1>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium tracking-wider">x Supply Chain</p>
          </div>
        </div>
        <button onClick={toggleSidebar} className="lg:hidden text-slate-500 dark:text-slate-400 hover:text-red-500"><X size={24} /></button>
      </div>
      ...
      <nav className="flex-1 px-4 space-y-6 overflow-y-auto py-2 custom-scrollbar">
        {menuGroups.map((group, index) => (
          <div key={index}>
            <p className="px-4 text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{group.title}</p>
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-3 flex-shrink-0 bg-white dark:bg-slate-900">
        <button onClick={() => setIsDarkMode(!isDarkMode)} className="flex items-center justify-between w-full px-4 py-3 text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors">
          <span className="text-sm font-medium">Appearance</span>
          {isDarkMode ? <Moon size={18} className="text-yellow-400" /> : <Sun size={18} className="text-orange-400" />}
        </button>
        <button onClick={logout} className="flex items-center gap-3 w-full px-4 py-3 text-red-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400 rounded-xl transition-colors">
